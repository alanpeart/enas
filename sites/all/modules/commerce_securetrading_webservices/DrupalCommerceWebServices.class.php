...
class DrupalCommerceWebServices extends AbstractWebServices {
	...
	/**
	 * Store the MD and the orderreference so we can lookup the orderreference by the MD value in the term URL (see commerce_securetrading_webservices_3d_secure_view()).
	 */
	protected function before3dRedirectAction($params) {
		db_query("INSERT INTO {commerce_securetrading_webservices_3dsecure} (md, orderreference) VALUES(:md, :orderreference)", array('md' => $params['result']->md, 'orderreference' => $params['result']->orderreference));
	}
	
	protected function handleError($message, $params) {
		$_SESSION['securetrading_webservices_error'] = $message;
	...
	protected function processPayment($params) {
		$this->success = TRUE;
		commerce_securetrading_webservices_transaction($params['payment_method'], $params['order'], $params['charge'], $params['result']->transactionreference);
	...
	protected function process3dSecure($params) {
		commerce_securetrading_webservices_transaction($params['payment_method'], $params['order'], $params['charge'], $params['result']->transactionreference);
		commerce_checkout_complete($params['order']);
		db_query('DELETE FROM {commerce_securetrading_webservices_3dsecure} WHERE orderreference = :orderreference', array(':orderreference' => $params['result']->orderreference));
